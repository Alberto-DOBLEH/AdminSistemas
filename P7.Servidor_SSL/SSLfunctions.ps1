...
function Configsslftp{
    ...
    $cert = Get-Item "Cert:\LocalMachine\My\DAA0DD1A9B0701DFC897662244BF9F63BF72B6A2" #Selecciona el certificado, si generaste otro cambia la ultima parte de la ruta por el nuevo
    echo $cert
    Set-ItemProperty "IIS:\Sites\PruebaFTP" -Name "ftpServer.security.ssl.serverCertHash" -Value DAA0DD1A9B0701DFC897662244BF9F63BF72B6A2 #Asigne a mano el ssl porque ajá pero puedes hacerlo con un comando que sale mas abajo
...
function ConfigsslIIS{
    ...
    $cert = Get-Item "Cert:\LocalMachine\My\DAA0DD1A9B0701DFC897662244BF9F63BF72B6A2"
...
function ConfigsslCaddy{
    ...
    $cert = Get-Item "Cert:\LocalMachine\My\DAA0DD1A9B0701DFC897662244BF9F63BF72B6A2"
    Export-PfxCertificate -Cert $cert -FilePath C:\caddy\certificado.pfx -Password (ConvertTo-SecureString -String "Rubo2004" -Force -AsPlainText)
    Export-Certificate -Cert $cert -FilePath "C:\caddy\certificado.crt"
    openssl pkcs12 -in C:\caddy\certificado.pfx -nocerts -nodes -out C:\caddy\clave.key -passin pass:Rubo2004
