...
function Comprobarpuerto{
  ...
  if($newPort -gt 1 -and $newPort -lt 65535 -and $puertosinvalidos -notcontains $newPort){
    if($netstatoutput){
        return $false
        Write-Host "El puerto est� en uso"
      }else{
        return $true
        Write-Host "Puerto Valido, se proceder� a la instalacion"
      }
  }else{
    return $false
        ...
        switch($opc){
            '1'{
                if(Test-Path "C:\caddy\caddy.exe"){
                    Write-Host "Caddy ya est� instalado en el equipo"
                }else{
                    while (-not $puertovalido){
                        $newPort = Read-Host "Introduce el puerto donde correr� el servicio"
                        if(Comprobarpuerto -newPort $newPort){
                            $puertovalido = $true
                            Write-Host "Puerto Valido, se proceder� a la instalacion"
                        }else{
                            $puertovalido = $false
                            Write-Host "Puerto invalido o est� en uso ingresa otro dato"
                        }
                    }
            ...
            '2'{
                ...
                if(Test-Path "C:\nginx\nginx-*\nginx.exe"){
                    Write-Host "Nginx ya est� instalado en el equipo"
                }else{
                    while (-not $puertovalido){
                        $newPort = Read-Host "Introduce el puerto donde correr� el servicio"
                        if(Comprobarpuerto -newPort $newPort){
